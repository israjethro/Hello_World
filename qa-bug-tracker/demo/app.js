...
(function () {
  ...
  /* ============================================================
     0. DATA CLEANING — trim, canonical casing, de-duplication
     Fixes "duplicate / case-sensitive" splits (e.g. Humana vs humana,
     "Yogesh Kumar D" vs "Yogesh kumar d") and removes repeated rows.
     ============================================================ */
  var CANON_FIELDS = ["client", "qaResource", "productionResource", "requestor", "month", "project"];
  function cleanStr(v) { return v == null ? "" : String(v).replace(/\u00a0/g, " ").replace(/\s+/g, " ").trim(); }
  function canonicalize(rows) {
    rows = (rows || []).map(function (r) {
      var o = {};
      for (var k in r) {
        if (!r.hasOwnProperty(k)) continue;
        if (k === "bugReport") o.bugReport = { text: cleanStr(r.bugReport && r.bugReport.text), url: (r.bugReport && r.bugReport.url) || "" };
        else if (typeof r[k] === "string") o[k] = cleanStr(r[k]);
        else o[k] = r[k];
      }
      if (!o.bugReport) o.bugReport = { text: "", url: "" };
      return o;
    });
    // Merge values that differ only by case/spacing into the most common spelling.
    CANON_FIELDS.forEach(function (f) {
      var freq = {};
      rows.forEach(function (r) { var v = r[f]; if (!v) return; var key = v.toLowerCase();
        (freq[key] = freq[key] || {}); freq[key][v] = (freq[key][v] || 0) + 1; });
      var canon = {};
      Object.keys(freq).forEach(function (key) { var best = null, bc = -1;
        Object.keys(freq[key]).forEach(function (o) { if (freq[key][o] > bc) { bc = freq[key][o]; best = o; } }); canon[key] = best; });
      rows.forEach(function (r) { var v = r[f]; if (v) r[f] = canon[v.toLowerCase()] || v; });
    });
    // Drop repeated records (case-insensitive across every column except S NO).
    var seen = {}, out = [];
    rows.forEach(function (r) {
      var sig = COLUMNS.filter(function (c) { return c.key !== "sno"; }).map(function (c) {
        if (c.key === "bugReport") return r.bugReport ? (r.bugReport.text + "|" + r.bugReport.url) : "";
  ...
  function initNav() {
    ...
    links.forEach(function (l) {
      l.addEventListener("click", function (e) {
        ...
        if (href.charAt(0) === "#") {
          e.preventDefault();                       // stop hash navigation (blanks the Apps Script iframe)
          var target = document.querySelector(href);
          if (target) {
            target.classList.add("in");             // ensure the section is revealed before we jump
  ...
  function tagBg(tag) { return tag === "b-hi" ? "rgba(255,92,124,.18)" : tag === "b-mid" ? "rgba(255,180,67,.18)" : "rgba(39,225,163,.18)"; }
  function taskCard(r) {
    var tag = severity(r.issues);
    return '<div class="task">' +
      '<div class="row1"><span class="tag ' + tag + '" style="background:' + tagBg(tag) + '">' + esc(r.client || "—") + '</span>' +
      '<span class="badge-num ' + tag + '">' + esc(r.issues) + ' 🐞</span></div>' +
      '<h4>' + esc(r.project && r.project !== "NA" ? r.project : String(r.typeOfQA || "").split(",")[0]) + '</h4>' +
  ...
  var RES_TASK_LIMIT = 6;          // tasks shown per engineer before "Show all"
  var resExpanded = {};
  function renderResources() {
    ...
    $("#resWrap").innerHTML = groups.map(function (g) {
      var sorted = g.rows.slice().sort(function (a, b) { return num(b.issues) - num(a.issues); });
      var expanded = !!resExpanded[g.name];
      var shown = expanded ? sorted : sorted.slice(0, RES_TASK_LIMIT);
      var tasks = shown.map(taskCard).join("");
      var more = g.rows.length > RES_TASK_LIMIT
        ? '<div class="task-more"><button class="btn ghost" data-res="' + esc(g.name) + '">' + (expanded ? "Show less ▲" : "Show all " + g.rows.length + " tasks ▼") + "</button></div>"
        : "";
      return '<div class="res-card">' +
        ...
        '<div class="task-grid">' + (tasks || '<div class="empty">No tasks.</div>') + '</div>' + more + '</div>';
    }).join("") || '<div class="empty">No QA resources found.</div>';

    $$("#resWrap .task-more button").forEach(function (b) {
      b.addEventListener("click", function () { var n = b.getAttribute("data-res"); resExpanded[n] = !resExpanded[n]; renderResources(); });
    });
  }
  ...
  /* ============================================================
     ...
     ============================================================ */
  var tState = { search: "", client: "", qa: "", month: "", type: "", source: "", sortKey: "sno", sortDir: 1, page: 1, size: 15 };
  ...
  function filteredRows() {
    ...
    return DATA.filter(function (r) {
      ...
      if (tState.source && (r._source || "") !== tState.source) return false;
      if (!q) return true;
  ...
  function initTableControls() {
    ...
    // Source/Sheet filter — only shown when records come from more than one sheet (multi-sheet build).
    var sources = uniq(DATA.map(function (r) { return r._source; }).filter(Boolean)).sort();
    var srcSel = $("#fSource");
    if (srcSel) {
      if (sources.length > 1) { srcSel.innerHTML = opts(sources, "All Sheets"); srcSel.style.display = ""; }
      else { srcSel.style.display = "none"; }
      srcSel.addEventListener("change", function (e) { tState.source = e.target.value; tState.page = 1; renderTable(); });
    }
    ...
    $("#clearFilters").addEventListener("click", function () {
      tState = { search: "", client: "", qa: "", month: "", type: "", source: "", sortKey: "sno", sortDir: 1, page: 1, size: tState.size };
      $("#globalSearch").value = ""; $("#fClient").value = ""; $("#fQA").value = ""; $("#fMonth").value = ""; $("#fType").value = "";
      if ($("#fSource")) $("#fSource").value = ""; renderTable();
    });
  ...
  function revealEl(el) {
    el.classList.add("in");
    $$("[data-c]", el).forEach(function (x) { if (!x.dataset.done) { x.dataset.done = 1; countUp(x, +x.getAttribute("data-c")); } });
  }
  function initReveal() {
    var els = $$(".reveal");
    ...
    if (!("IntersectionObserver" in window)) { els.forEach(revealEl); }
      ...
      // threshold 0 = reveal as soon as ANY pixel enters the viewport. This is the key fix:
      // very tall sections (e.g. QA Resources) could never reach the old 12% threshold and stayed hidden.
      _revealIO = new IntersectionObserver(function (entries) {
        entries.forEach(function (en) { if (en.isIntersecting) { revealEl(en.target); _revealIO.unobserve(en.target); } });
      }, { threshold: 0, rootMargin: "0px 0px -5% 0px" });
      els.forEach(function (el) { if (!el.classList.contains("in")) _revealIO.observe(el); });
      // Safety net: never leave a section hidden, whatever its height.
      setTimeout(function () { $$(".reveal").forEach(function (el) { if (!el.classList.contains("in")) revealEl(el); }); }, 1400);
    }
  ...
  window.__setData = function (rows, cfg) { DATA = canonicalize(rows || []); if (cfg) { window.APP_CONFIG = CFG = cfg; } render(); };
})();
